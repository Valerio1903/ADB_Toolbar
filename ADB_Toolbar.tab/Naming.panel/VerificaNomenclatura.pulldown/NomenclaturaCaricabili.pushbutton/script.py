...
for Elemento in Collector_IFC_Valorizzato:
    ...
    if Valore_IFCExportAs not in DizionarioDiVerifica.keys():
        ...
        # PENSARE DI DIVIDERE GLI SCRIPT TRA CARICABILI E SISTEMA
        if not EstraiInfoOggetto(Elemento)[0]:
            pass
        # QUI METTERE IL CODICE PER LA VERIFICA DELLE FAMIGLIE DI SISTEMA
        # CHE SARA SEMPLIFICATA E MENO LUNGA DI QUESTA
        else:
            # ESTRAGGO LE INFORMAZIONI DALL'ELEMENTO
            Scomposizione_Nome_Famiglia = EstraiInfoOggetto(Elemento)[0].split("_")
            Nome_Famiglia = EstraiInfoOggetto(Elemento)[0]
            Nome_Tipo = EstraiInfoOggetto(Elemento)[1].split("_")
            ...
            try:
                ...
                if not VerificaCodifica(Nome_Famiglia, V0_VerificaCampi)[0]:
                    print(VerificaCodifica(Nome_Famiglia, V0_VerificaCampi)[1])
                    pass
                else:
                    ...
                    for index,Check in enumerate(CheckStatus):
                        ...
                        # PROCEDO CON LA VERIFICA DEL NOME DEL TIPO
                        Scomposizione_Nome_Famiglia_Intero = EstraiInfoOggetto(Elemento)[0]
                        Nome_Tipo_Intero = EstraiInfoOggetto(Elemento)[1]

                        if Scomposizione_Nome_Famiglia_Intero not in Nome_Tipo_Intero:
                            print("Nomenclatura Errata, Nome Famiglia non presente nel Nome del Tipo come da specifiche. {}".format(EstraiInfoOggetto(Elemento)[1]))
                        else:
                            Composizione_Tipo = Nome_Tipo_Intero.split(Scomposizione_Nome_Famiglia_Intero)[1].split("_")
                            if len(Composizione_Tipo[1]) <= 25 and len(Composizione_Tipo[2]) <= 9:
                                print("Nomenclatura Tipo corretta, {} ".format(EstraiInfoOggetto(Elemento)[1]))
                            else:
                                if len(Composizione_Tipo[1]) > 25:
                                    print("Caratteristica troppo lunga, ridurre la lunghezza per un max. 25 caratteri. {}".format(Composizione_Tipo[1]))
  
                                if len(Composizione_Tipo[2]) > 9:
                                    print("Dimensioni troppo lunghe, ridurre la lunghezza per un max. 9 caratteri. {}".format(Composizione_Tipo[2]))
            except Exception as e:
                print(e)
                pass